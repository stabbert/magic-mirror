...
function calculateTimeTitle(nowTimeInMs, newEvent) {
  if (newEvent.fullDayEvent) {
    ...
    if (
      isSame(newEvent.startTimeInMs, startTimeInMsOnlyDate) &&
      isSame(newEvent.endTimeInMs, endTimeInMsOnlyDate)
    ) {
  ...
  if (isAfter(newEvent.startTimeInMs, nowTimeInMs)) {
    ...
    if (diffInMs < twoDayInMs) {
      ...
      const startTimeInMsOnlyDate = startOfDayAsTimeInMs(
        newEvent.startTimeInMs
      );
      const nowTimeInMsOnlyDate = startOfDayAsTimeInMs(nowTimeInMs);
...
export default {
  ...
  created() {
    ...
    function updateCalendar() {
      ...
      const futureTimeInMs =
        addDaysAsTimeInMs(nowTimeInMs, config.maximumNumberOfDays) - 1000; // Subtract 1 second so that events that start on the middle of the night will not repeat.
            ...
            .then((data) => {
              ...
              for (let index in allComp) {
                ...
                if (event.isRecurring()) {
                  ...
                  let shiftedTimeInMs = shiftedRecurrenceEvent
                    ? toTimeInMs(shiftedRecurrenceEvent.recurrenceId)
                    : null;
                  ...
                  while (nextTimeInMs) {
                    if (
                      shiftedTimeInMs &&
                      isSame(nextTimeInMs, shiftedTimeInMs)
                    ) {
                    ...
                    if (
                      timesInMs.length <= config.maximumEntries &&
                      isBefore(nextTimeInMs, futureTimeInMs)
                    ) {
                      nextTimeInMs = fromRuleNextTimeInMs(rule);
                    } else {
                      nextTimeInMs = false;
      ...
      Promise.all(calendarFetches).then((allCalendarNewEvents) => {
        ...
        function updateTimeInAllEvents() {
          ...
          maximumAllNewEvents.forEach((newEvent, index) => {
            self.events[index].time = calculateTimeTitle(nowTimeInMs, newEvent);
          });
        }
